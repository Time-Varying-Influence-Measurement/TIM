...
class IcmlInfluenceCalculator(InfluenceCalculator):
    ...
    def calculate(self) -> tuple[np.ndarray, list]:
        ...
        ]  # Should already be on device? Double check.
        self.logger.info("Computed initial gradient 'u' from validation set.")

        # --- Optimization to find v ≈ H^-1 u ---
        # Use specific alpha for ICML? Original code has special handling for dnn.
        # Using self.alpha from global_info for now.
        alpha_icml = self.alpha
        # alpha_icml = 1.0 if self.model_type == "dnn" else self.alpha # As per original code snippet if needed
        # logger.info(f"Using alpha = {alpha_icml} for ICML optimization.")

        num_steps_icml = int(np.ceil(self.n_tr / BATCH_SIZE_ICML))
        # Initialize v = u (start optimization from u)
        v = [uu.clone().detach().requires_grad_(True) for uu in u]
        optimizer = optim.SGD(v, lr=LR_ICML, momentum=MOMENTUM_ICML)
        loss_train_icml = []  # To store optimization loss history
        ...
        for epoch in range(NUM_EPOCHS_ICML):
            ...
            # Shuffling handled inside the loop based on epoch seed
            np.random.seed(epoch)  # Seed for shuffling for this epoch
            idx_list = np.array_split(np.random.permutation(self.n_tr), num_steps_icml)

            epoch_loss_sum = 0.0
            for i, idx_batch in enumerate(idx_list):
                # Convert numpy indices to tensor
                ...
                # Note: Original ICML paper/code often doesn't add L2 regularization here
                # when calculating the Hessian-vector product implicitly. Sticking to original snippet's logic.

                model.zero_grad()  # Clear gradients on the model
                # grad_params = H*v requires gradient w.r.t model parameters
                grad_params = torch.autograd.grad(
                    loss, model.parameters(), create_graph=True
                )

                # vg = v dot grad(L_train_batch)
                vg = sum((vv * g).sum() for vv, g in zip(v, grad_params))

                # vgrad_params = H_train_batch * v (gradient of vg w.r.t model parameters)
                # Need to retain graph for v if higher-order derivatives needed, but seems not here.
                # Detach model params grad to compute grad w.r.t v only for the optimizer step.
                vgrad_params = torch.autograd.grad(
                    vg, model.parameters(), create_graph=False
                )  # Or retain_graph=True if needed later?

                # Compute the loss for the optimizer (objective to minimize)
                # Loss = 0.5 * v^T H v - u^T v (approx) + 0.5 * alpha * v^T v (L2 reg on v itself)
                # loss_i = 0.5 * vg_dot_v + 0.5 * alpha * v_dot_v - u_dot_v
                # The provided code calculates loss_i = sum(0.5 * (vgp * vv + alpha_icml * vv * vv).sum() - (uu * vv).sum())
                # Let's verify: term1 = sum(0.5*vgp*vv) = 0.5 * v * H * v ? Not quite.
                ...
                optimizer.zero_grad()  # Zero gradients for v parameters
                loss_i.backward()  # Compute gradients of loss_i w.r.t v
                optimizer.step()  # Update v
