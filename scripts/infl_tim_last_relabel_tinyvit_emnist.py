...
def setup_directories(network, relabel):
    ...
    save_dir = f"{hessian_dir}/emnist_{network}_tim_last_tracin_relabel_{relabel}_10e-1"
...
def run_tim_last_influence(network, gpu, save_dir, relabel):
    """Run the tim_last influence computation."""
...
def copy_plots(hessian_dir, dest_dir, save_dir, sec71_save_dir, relabel):
    ...
    pairwise_plot_names = [
        f"cleansing_plot_{relabel}_pct_tracin_vs_tim_last_asc.png",
        f"cleansing_plot_{relabel}_pct_tracin_vs_tim_last_desc.png"
    ]

    print(f"Copying pairwise comparison plots with TracIn")
...
def run_configuration(config_idx, gpu_ids):
    try:
        ...
        # run_tim_last_influence(network, gpu, save_dir, relabel)
